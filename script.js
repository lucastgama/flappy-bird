...
function Passaro(alturaJogo) {
  ...
  this.animar = () => {
    const novoY = this.getY() + (voando ? 6 : -6);
...
function estaoSobrepostos(elementoA, elementoB) {
  ...
  console.log(a && b);
  const horizontal = a.left + a.window >= b.left && b.left + b.window >= a.left;
...
function GameManager() {
  ...
  const areaDoJogo = document.querySelector("[gameBoard]");
  const altura = areaDoJogo.clientHeight;
  const largura = areaDoJogo.clientWidth;

  const progresso = new Progresso();
  const barreiras = new Barreiras(altura, largura, 100, 200, () => {
